...
const CandidateDashboard = ({ user }) => {
  ...
  const fetchDashboardData = async () => {
    try {
      const [appsRes, notifRes, docsRes, payRes] = await Promise.all([
        fetch(`/api/candidate/applications?userId=${user.id}`),
        fetch(`/api/candidate/notifications?userId=${user.id}`),
        fetch(`/api/candidate/documents?userId=${user.id}`),
        fetch(`/api/candidate/payments?userId=${user.id}`)
      ]);

      const [apps, notifs, docs, pays] = await Promise.all([
        appsRes.json(),
        notifRes.json(),
        docsRes.json(),
        payRes.json()
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">
            Suivez vos demandes et documents en temps réel
          </p>
        </div>
